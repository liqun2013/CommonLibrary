...
namespace CommonLibrary
{
	public static class TypeExtenssions
	{
		...
		public static T ToDto<T>(this BaseEntity entity) where T : BaseDto, new()
		{
			T dto = new T();

			PropertyInfo[] dtoProperties = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.SetProperty);
			PropertyInfo[] entityProperties = entity.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance);

			string mapto = string.Empty;
			string pName = string.Empty;
			PropertyInfo ep = null;
			object ev = null;
			foreach (var p in dtoProperties)
			{
				mapto = getMapto(p);
				pName = string.IsNullOrWhiteSpace(mapto) ? p.Name : mapto;

				ep = entityProperties.FirstOrDefault(x => x.Name.Equals(pName, StringComparison.OrdinalIgnoreCase));
				if (ep != null)
				{
					ev = ep.GetValue(entity);
					if (ev != null)
					{
						Type ptype = p.PropertyType;
						if (ptype.IsAssignableFrom(ev.GetType()))
							p.SetValue(dto, ev);
						else
						{
							...
							SetPropertyValue(ev.ToString(), dto, p, ptype);
						}
					}
				}
			}
		...
		private static string getMapto(PropertyInfo p)
		{
			var dtoAttribute = p.GetCustomAttribute<DtoMapperAttrbute>();
			if (dtoAttribute != null)
				return dtoAttribute.Mapto;

			return string.Empty;
		}
		private static void SetPropertyValue(string v, BaseDto result, PropertyInfo p, Type ptype)
		{
			switch (ptype.Name.ToLower())
			{
				...
				case "datetime":
					{
						p.SetValue(result, v.ToDateTime(true), null);
					}
					break;
				case "int32":
					{
						p.SetValue(result, v.ToInt32(true), null);
					}
					break;
				case "uint32":
					{
						p.SetValue(result, v.ToUInt32(true), null);
					}
					break;
				case "int16":
					{
						p.SetValue(result, v.ToInt16(true), null);
					}
					break;
				case "uint16":
					{
						p.SetValue(result, v.ToUInt16(true), null);
					}
					break;
				case "int64":
					{
						p.SetValue(result, v.ToInt64(true), null);
					}
					break;
				case "uint64":
					{
						p.SetValue(result, v.ToUInt64(true), null);
					}
					break;
				case "boolean":
					{
						p.SetValue(result, v.ToBoolean(true, false), null);
					}
					break;
				case "double":
					{
						p.SetValue(result, v.ToDouble(true), null);
					}
					break;
				case "decimal":
					{
						p.SetValue(result, v.ToDecimal(true), null);
					}
					break;
				case "float":
					{
						p.SetValue(result, v.ToFloat(true), null);
					}
					break;
				case "char":
					{
						p.SetValue(result, v.ToChar(true, char.MinValue), null);
					}
					break;
			}
		}
